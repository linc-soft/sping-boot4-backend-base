-- ============================================================
-- Yearly Partition Schema
-- Recommended for: Medium-volume logging (<100k rows/day)
--
-- Notes:
...
--      partition-maintenance.sql and call sp_create_yearly_partitions
...
--     p_init: VALUES LESS THAN (2027)
--     -> Accepts all data before year 2027 (including historical data)
--   p_future is the catch-all partition for data beyond existing ranges
-- ============================================================
-- ============================================================
-- sys_access_log table (yearly partition)
...
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'Access log (yearly partition)'
PARTITION BY
    RANGE (YEAR (create_time)) (
        PARTITION p_init
        VALUES
            LESS THAN (2027) COMMENT 'Initial partition (includes historical data)',
            PARTITION p_future
        VALUES
...
-- ============================================================
-- sys_error_log table (yearly partition)
...
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'Error log (yearly partition)'
PARTITION BY
    RANGE (YEAR (create_time)) (
        PARTITION p_init
        VALUES
...
-- ============================================================
-- sys_operation_log table (yearly partition)
...
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'Operation log (yearly partition)'
PARTITION BY
    RANGE (YEAR (create_time)) (
        PARTITION p_init
        VALUES
